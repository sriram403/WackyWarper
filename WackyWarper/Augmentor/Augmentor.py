...
# Each worker process does its own cv2/albumentations calls; without this,
# OpenCV's internal thread pool multiplies on top of our process pool and
# oversubscribes the CPU, making things slower rather than faster.
cv2.setNumThreads(1)
...
def _augment_one(partition:str, image:str, folder_name:str, images_needed:int):
    cv2.setNumThreads(1)
    img = cv2.imread(os.path.join(partition, 'images', image))
    coords_list = []
    class_value = []
...
def New_Start_Augmentor(list_of_directory:list, header_folder_name:str, images_needed:int, max_workers:int=None):
    '''
    list_of_directory-> list of directories where images and labels are located
    header_folder_name-> output folder name
    images_needed-> how many augmented images per original image
    max_workers-> worker processes to use (default: all CPU cores on the instance)
    '''
    print(">>Augmentation started<<")
    folder_name = header_folder_name
    ...
    for partition in list_of_directory:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(_augment_one, partition, image, folder_name, images_needed)
                for image in image_list
            ]
            for future in tqdm(as_completed(futures), total=len(futures), desc=os.path.basename(partition), unit="img"):
                future.result()
    print(">>Augmentation Ended<<")
